...
#include	"../_libs/crc16.h"
#include	"../_libs/string.h"
#include    "config.h"
...
#define RxData RA1
#define TxData RA2
#define RxTris TRISA1
#define TxTris TRISA2
...
static boolean vw_tx_header; // flag to decide if transmitting header or payload
...
static boolean vw_tx_enabled = 0; // Flag tx enabled?
...
// RX
// -----------------------------------------------------------------------------
static volatile boolean vw_rx_done = 0; // flag new message is available
static uint16_t vw_rx_bits = 0; // last 12 bits to look for the start symbol
static boolean vw_rx_enabled = 0; // flag receiver enabled
static boolean vw_rx_enabled_last = 0; // flag receiver enabled reposition
...
static boolean vw_rx_active = 0; // start sequence found and now decoding data

static boolean vw_rx_inverted = 0;
// -----------------------------------------------------------------------------
// PLL
// -----------------------------------------------------------------------------
...
static boolean _calc_timer0_prescaler(uint16_t brate, uint16_t *prescaler_value, uint8_t *prescaler_bits, uint8_t *k_tmr0)
{
...
void vw_setup(uint16_t brate)
{
    ...
    {
        // prescaler needed
        OPTION_REG &= 0xF0; // all 4 right bits to 0 (PSA=0 -> prescaler for timer0)
        OPTION_REG |= prescaler_bits;
    }
...
boolean vw_send(const char *buf, uint8_t len)
{
    uint8_t i;
    uint8_t index = 0;
    uint16_t crc = 0xffff;
...
boolean vw_have_message(void)
{
    return (vw_rx_done) ? 1 : 0;
}

boolean vw_recv(uint8_t *buf, uint8_t *len)
{
    uint8_t rxlen;
...
void vw_set_rx_inverted(void){
	vw_rx_inverted = 1;
}

boolean vw_is_rx_inverted(void){
	return vw_rx_inverted;
}