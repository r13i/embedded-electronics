...
#define SEND_PERIOD 1   // Seconds. Send via bluetooth each SEND_PERIOD seconds


/*************************************************************************************************/
// Set Gas threshold
#define GAS_THRESHOLD 350       // Format to fit the range [0 - 1024] of the 10-bit ADC
...
void interrupt ISR(){
    ...
    if(TMR0IE && TMR0IF){       // If a Timer 0 overflow occurs, the flag TMR0IF rises
        ...
        // Send sensors data via Bluetooth each SEND_PERIOD seconds
        if(timer0_count >= ONE_SEC * SEND_PERIOD){
            ...
            for(j = 0; j < N_SENSORS; ++j){
                char buffer[30];    // A string buffer to hold the data
                sprintf(buffer, "%s: %lu / 100", SENSOR_TYPES[j], (unsigned long) ADC_Read(SENSOR_CHANNELS[j]) * 100 / 1024);
                
                // Send the string via UART to Bluetooth module, which will send them wirelessly
